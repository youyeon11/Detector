...
"""
Elasticsearch Index 자동 생성 스크립트
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Create Elasticsearch indices from JSON definitions.")
    parser.add_argument(
        "--recreate",
        action="store_true",
        help="Delete an index first when it already exists.",
    )
...
def main() -> int:
    ...
    index_specs = [
        (
            settings.profanity_lexicon_index,
            ROOT / "elastic" / "profanity_lexicon_index.json",
        ),
        (
            settings.noisy_text_docs_index,
            ROOT / "elastic" / "noisy_text_docs_index.json",
        ),
    ]
